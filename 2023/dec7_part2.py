...
with open(input_file_path, "r", encoding="utf-8") as f:
    ...
    for hand in game:
        i=hand
        i=i.replace("0",max(set(i)-{0}, key=i.replace("0","").count))
        if i.count(i[0]) == 5:
            kind5.append(hand)
            kind5amt.append(game[hand])
        elif i.count(i[0]) == 4 or i.count(i[-1]) == 4:
            kind4.append(hand)
            kind4amt.append(game[hand])
            ...
            if i.count(i[0]) == 2 or i.count(i[-1]) == 2 or i.count(i[1]) == 2 or i.count(i[2]) == 2 or i.count(i[3]) == 2 or i.count(i[4]) == 2:
                ...
                full_house_amt.append(game[hand])
            else:
                kind3.append(hand)
                kind3amt.append(game[hand])
        else:
            x="".join(sorted(i))
            #print(x)
            if (x.count(x[0]) == 2 and x.count(x[2]) == 2 ) or (x.count(x[1]) == 2 and x.count(x[3]) == 2 ):
                ...
                two_pair_amt.append(game[hand])
            elif x.count(x[0]) == 2 or x.count(x[1]) == 2 or x.count(x) == 2 or x.count(x[3]) == 2:
                one_pair.append(hand)
                one_pair_amt.append(game[hand])
            else:
                high_card.append(i)
                high_card_amt.append(game[hand])
